...
class SyncBatchNormAct(nn.SyncBatchNorm):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = super().forward(
            x)  # SyncBN doesn't work with torchscript anyways, so this is fine
...
def convert_sync_batchnorm(module, process_group=None):
    ...
    for name, child in module.named_children():
        module_output.add_module(name,
                                 convert_sync_batchnorm(child, process_group))
    del module
...
class GroupNormAct(nn.GroupNorm):
    ...
    def forward(self, x):
        if self._fast_norm:
            ...
            x = F.group_norm(x, self.num_groups, self.weight, self.bias,
                             self.eps)
        x = self.drop(x)
        x = self.act(x)
        return x
...
class LayerNormAct(nn.LayerNorm):
    ...
    def __init__(self,
                 ...
                 inplace=True,
                 drop_layer=None):
        super(LayerNormAct, self).__init__(normalization_shape,
                                           eps=eps,
...
class LayerNormAct2d(nn.LayerNorm):
    ...
    def __init__(self,
                 ...
                 inplace=True,
                 drop_layer=None):
        super(LayerNormAct2d, self).__init__(num_channels,
                                             eps=eps,
                                             elementwise_affine=affine)
        self.drop = drop_layer() if drop_layer is not None else nn.Identity()
        act_layer = get_act_layer(act_layer)  # string -> nn.Module
