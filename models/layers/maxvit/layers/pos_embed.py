...
def pixel_freq_bands(
    ...
    if linear_bands:
        bands = torch.linspace(1.0,
                               max_freq / 2,
                               num_bands,
                               dtype=dtype,
                               device=device)
    else:
        bands = 2**torch.linspace(0,
                                  math.log(max_freq, 2) - 1,
                                  num_bands,
                                  dtype=dtype,
                                  device=device)
...
def inv_freq_bands(
    ...
    inv_freq = 1. / (temperature**(torch.arange(
        0, num_bands, step, dtype=dtype, device=device) / num_bands))
    return inv_freq
...
def build_sincos2d_pos_embed(
    ...
    pos_emb = torch.stack([torch.sin(pos2), torch.cos(pos2)],
                          dim=stack_dim).flatten(1)
    return pos_emb
...
def build_fourier_pos_embed(
    ...
    if bands is None:
        if in_pixels:
            bands = pixel_freq_bands(num_bands,
                                     float(max_res),
                                     linear_bands=linear_bands,
                                     dtype=dtype,
                                     device=device)
        else:
            bands = inv_freq_bands(num_bands,
                                   step=1,
                                   dtype=dtype,
                                   device=device)
    else:
    ...
    if in_pixels:
        grid = torch.stack(torch.meshgrid([
            torch.linspace(-1., 1., steps=s, device=device, dtype=dtype)
            for s in feat_shape
        ]),
                           dim=-1)
    else:
...
class FourierEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = build_fourier_pos_embed(feat_shape,
                                      self.bands,
        ...
        if self.keep_spatial:
            x = torch.cat(
                [x,
                 emb.unsqueeze(0).expand(batch_expand).permute(0, 3, 1, 2)],
                dim=1)
        else:
            x = torch.cat(
                [x.permute(0, 2, 3, 1),
                 emb.unsqueeze(0).expand(batch_expand)],
                dim=-1)
...
def build_rotary_pos_embed(
    ...
    sin_emb, cos_emb = build_fourier_pos_embed(feat_shape,
                                               bands=bands,
                                               num_bands=dim // 4,
                                               max_res=max_freq,
                                               linear_bands=linear_bands,
