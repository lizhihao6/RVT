...
class GroupNorm(nn.GroupNorm):
    ...
    def forward(self, x):
        if self.fast_norm:
            return fast_group_norm(x, self.num_groups, self.weight, self.bias,
                                   self.eps)
        else:
...
class LayerNorm(nn.LayerNorm):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self._fast_norm:
            x = fast_layer_norm(x, self.normalized_shape, self.weight,
                                self.bias, self.eps)
        else:
...
@torch.jit.script
def _layer_norm_cf(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                   eps: float):
...
def _layer_norm_cf_sqm(x: torch.Tensor, weight: torch.Tensor,
...
class LayerNormExp2d(nn.LayerNorm):
    """LayerNorm for channels_first tensors with 2d spatial dimensions (ie N,
    C, H, W).
    ...
    def forward(self, x) -> torch.Tensor:
        if _is_contiguous(x):
            x = F.layer_norm(x.permute(0, 2, 3,
