...
def inv_instance_rms(x, eps: float = 1e-5):
    rms = x.square().float().mean(dim=(2, 3),
                                  keepdim=True).add(eps).rsqrt().to(x.dtype)
...
class FilterResponseNormTlu2d(nn.Module):
    ...
    def __init__(self, num_features, apply_act=True, eps=1e-5, rms=True, **_):
        ...
        self.tau = nn.Parameter(
            torch.zeros(num_features)) if apply_act else None
    ...
    def forward(self, x):
        ...
        x = x * self.weight.view(v_shape).to(
            dtype=x_dtype) + self.bias.view(v_shape).to(dtype=x_dtype)
        return torch.maximum(x,
                             self.tau.reshape(v_shape).to(
                                 dtype=x_dtype)) if self.tau is not None else x
