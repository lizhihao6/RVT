...
class PosEmbedRel(nn.Module):
    """Relative Position Embedding As per: https://gist.github.com/aravindsrini
    vas/56359b79f0ce4449bcb04ab4b56a57a2.
    ...
    def __init__(self, feat_size, dim_head, scale):
        ...
        self.width_rel = nn.Parameter(
            torch.randn(self.width * 2 - 1, dim_head) * scale)
...
class BottleneckAttn(nn.Module):
    ...
    def __init__(self,
        ...
        self.qkv = nn.Conv2d(dim,
                             self.dim_out_qk * 2 + self.dim_out_v,
                             1,
                             bias=qkv_bias)

        # NOTE I'm only supporting relative pos embedding for now
        self.pos_embed = PosEmbedRel(feat_size,
    ...
    def forward(self, x):
        ...
        q = q.reshape(B * self.num_heads, self.dim_head_qk,
                      -1).transpose(-1, -2)
        k = k.reshape(B * self.num_heads, self.dim_head_qk,
                      -1)  # no transpose, for q @ k
        v = v.reshape(B * self.num_heads, self.dim_head_v,
                      -1).transpose(-1, -2)
        ...
        out = (attn @ v).transpose(-1, -2).reshape(B, self.dim_out_v, H,
                                                   W)  # B, dim_out, H, W
        out = self.pool(out)
        return out
