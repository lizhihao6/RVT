...
from data.genx_utils.sequence_for_streaming import (RandAugmentIterDataPipe,
                                                    SequenceForIter)
...
def build_streaming_dataset(dataset_mode: DatasetMode, dataset_config: DictConfig, batch_size: int, num_workers: int) \
    ...
    for entry in tqdm(
        ...
        new_datapipes = get_sequences(path=entry,
    ...
    if dataset_mode == DatasetMode.TRAIN:
        return build_streaming_train_dataset(datapipes=datapipes,
                                             dataset_config=dataset_config,
                                             batch_size=batch_size,
                                             num_workers=num_workers)
    elif dataset_mode in (DatasetMode.VALIDATION, DatasetMode.TESTING):
        return build_streaming_evaluation_dataset(datapipes=datapipes,
                                                  batch_size=batch_size)
    else:
        raise NotImplementedError


def get_sequences(path: Path, dataset_config: DictConfig,
                  guarantee_labels: bool) -> List[SequenceForIter]:
    assert path.is_dir()
...
def build_streaming_train_dataset(datapipes: List[MapDataPipe],
    ...
    augmentation_datapipe_type = partialclass(RandAugmentIterDataPipe,
                                              dataset_config=dataset_config)
...
def build_streaming_evaluation_dataset(
    ...
    assert len(datapipes) > 0
    streaming_dataset = ShardedStreamingDataPipe(datapipe_list=datapipes,
                                                 batch_size=batch_size)
    return streaming_dataset
