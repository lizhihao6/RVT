...
class ShardedStreamingDataPipe(IterDataPipe):
    ...
    def __init__(self,
        ...
        self.datapipe_list = sorted(datapipe_list,
                                    key=lambda x: len(x),
                                    reverse=True)
    ...
    def get_zipped_stream_from_worker_datapipes(
            self, datapipe_list: List[MapDataPipe],
            batch_size: int) -> ZipperLongest:
        ...
        datapipe_list = sorted(datapipe_list,
                               key=lambda x: len(x),
                               reverse=True)
        zipped_streams = [[] for _ in range(batch_size)]
        batch_id_generator = self.yield_pyramid_indices(start_idx=0,
                                                        end_idx=batch_size)
        ...
        for idx, streams in enumerate(zipped_streams):
            zipped_streams[idx] = Concater(*(stream.to_iter_datapipe()
