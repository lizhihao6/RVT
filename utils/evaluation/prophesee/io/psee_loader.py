...
class PSEELoader(object):
    ...
    def __init__(self, datfile):
        ...
        assert self._extension in ['dat', 'npy'
                                   ], 'input file path = {}'.format(datfile)
        ...
        self._start, self.ev_type, self._ev_size, self._size = self._binary_format.parse_header(
            self._file)
    ...
    def load_n_events(self, ev_count):
        ...
        if ev_count >= count:
            self.done = True
            ev_count = count
            self._binary_format.stream_td_data(self._file, event_buffer,
                                               self._dtype, ev_count)
            self.current_time = event_buffer['t'][ev_count - 1] + 1
        else:
            self._binary_format.stream_td_data(self._file, event_buffer,
                                               self._dtype, ev_count + 1)
    ...
    def load_delta_t(self, delta_t):
        ...
        if delta_t < 1:
            raise ValueError(
                'load_delta_t(): delta_t must be at least 1 micro-second: {}'.
                format(delta_t))
        ...
        while tmp_time < final_time and pos < self._end:
            count = (min(self._end, pos + batch * self._ev_size) -
                     pos) // self._ev_size
            buffer = np.empty((count, ), dtype=self._decode_dtype)
            self._binary_format.stream_td_data(self._file, buffer, self._dtype,
                                               count)
            tmp_time = buffer['t'][-1]
    ...
    def seek_event(self, ev_count):
        ...
        if ev_count <= 0:
            ...
            self.current_time = np.fromfile(
                self._file, dtype=self._dtype, count=1)['t'][0] + 1
        else:
            ...
            self.current_time = np.fromfile(self._file,
                                            dtype=self._dtype,
                                            count=1)['t'][0]
    ...
    def seek_time(self, final_time, term_criterion=100000):
        """go to the time final_time inside the file. This is implemented using
        ...
        self.seek_event(low)
        final_buffer = np.fromfile(self._file,
                                   dtype=self._dtype,
                                   count=high - low)['t']
