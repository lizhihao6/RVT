"""Defines some tools to handle events. In particular :
...
def reformat_boxes(boxes):
    """ReFormat boxes according to new rule This allows to be backward-
    compatible with imerit annotation.
...
def loaded_label_to_prophesee(loaded_labels: ObjectLabels) -> np.ndarray:
    ...
    for name in BBOX_DTYPE.names:
        if name == 'track_id':
            ...
            continue
        loaded_label_proph[name] = np.asarray(loaded_labels.get(name),
                                              dtype=BBOX_DTYPE[name])
...
def to_prophesee(loaded_label_list: LOADED_LABELS, yolox_pred_list: YOLOX_PRED_PROCESSED) -> \
    ...
    for loaded_labels, yolox_preds in zip(loaded_label_list, yolox_pred_list):
        ...
        if num_pred > 0:
            ...
            yolox_pred_proph['t'] = np.ones(
                (num_pred, ), dtype=BBOX_DTYPE['t']) * time
            yolox_pred_proph['x'] = np.asarray(yolox_preds[:, 0],
                                               dtype=BBOX_DTYPE['x'])
            yolox_pred_proph['y'] = np.asarray(yolox_preds[:, 1],
                                               dtype=BBOX_DTYPE['y'])
            yolox_pred_proph['w'] = np.asarray(yolox_preds[:, 2] -
                                               yolox_preds[:, 0],
                                               dtype=BBOX_DTYPE['w'])
            yolox_pred_proph['h'] = np.asarray(yolox_preds[:, 3] -
                                               yolox_preds[:, 1],
                                               dtype=BBOX_DTYPE['h'])
