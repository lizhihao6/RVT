...
EV_STRING = 'Event2D'
EV_STRINGS = [EV_STRING]


def load_td_data(filename, ev_count=-1, ev_start=0):
    """Loads TD data from files generated by the StreamLogger consumer for
    Event2D events [ts,x,y,p]. The type ID in the file header must be 0. args :
...
def _dat_transfer(dat, dtype, xyp=None):
    """Transfers the fields present in dtype from an old datastructure to a new
    datastructure xyp should be passed as a tuple args :
    ...
    if xyp and xyp_index == -1:
        ...
        return
    if xyp_index >= 0:
        dtype = dtype[:xyp_index] + [('x', 'i2'), ('y', 'i2'),
                                     ('p', 'i2')] + dtype[xyp_index + 1:]
...
def count_events(filename):
    """Returns the number of events in a dat file args :
    ...
    while not end_of_header:
        ...
        if first_item != '% ':
            ...
            if len(words) > 1:
                ...
                if words[1] == 'Height' or words[
                        1] == b'Height':  # compliant with python 3 (and python2)
                    size[0] = int(words[2])
                    header += ['Height', words[2]]
                if words[1] == 'Width' or words[
                        1] == b'Width':  # compliant with python 3 (and python2)
                    size[1] = int(words[2])
