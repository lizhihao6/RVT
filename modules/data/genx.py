...
from data.genx_utils.collate import (custom_collate_rnd,
                                     custom_collate_streaming)
from data.genx_utils.dataset_rnd import (CustomConcatDataset,
...
def get_dataloader_kwargs(dataset: Union[Dataset, CustomConcatDataset],
                          ...
                          dataset_config: DictConfig, batch_size: int,
    ...
    if dataset_mode == DatasetMode.TRAIN:
        ...
        if sampling_mode == DatasetSamplingMode.RANDOM:
            ...
            sampler = get_weighted_random_sampler(
                dataset) if use_weighted_rnd_sampling else None
            return dict(
                dataset=dataset,
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, dataset_config: DictConfig, num_workers_train: int,
                 num_workers_eval: int, batch_size_train: int,
                 batch_size_eval: int):
        super().__init__()
        ...
        assert self.eval_sampling_mode in (DatasetSamplingMode.STREAM,
    ...
    def set_mixed_sampling_mode_variables_for_train(self):
        ...
        self.sampling_mode_2_train_batch_size[
            DatasetSamplingMode.RANDOM] = bs_rnd
        self.sampling_mode_2_train_batch_size[
            DatasetSamplingMode.STREAM] = bs_str
        ...
        self.sampling_mode_2_train_workers[
            DatasetSamplingMode.RANDOM] = workers_rnd
        self.sampling_mode_2_train_workers[
            DatasetSamplingMode.STREAM] = workers_str
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        if stage == 'fit':
            if self.train_sampling_mode == DatasetSamplingMode.MIXED:
                ...
                self.sampling_mode_2_train_workers[
                    self.train_sampling_mode] = self.overall_num_workers_train
                self.sampling_mode_2_train_batch_size[
                    self.train_sampling_mode] = self.overall_batch_size_train
            ...
            if self.train_sampling_mode in (DatasetSamplingMode.RANDOM,
            ...
            if self.train_sampling_mode in (DatasetSamplingMode.STREAM,
                                            DatasetSamplingMode.MIXED):
    ...
    def train_dataloader(self):
        ...
        for sampling_mode, dataset in self.sampling_mode_2_dataset.items():
            train_loaders[sampling_mode] = DataLoader(**get_dataloader_kwargs(
                ...
                dataset_config=self.dataset_config,
                batch_size=self.
                sampling_mode_2_train_batch_size[sampling_mode],
    ...
    def val_dataloader(self):
        return DataLoader(
            **get_dataloader_kwargs(dataset=self.validation_dataset,
    ...
    def test_dataloader(self):
        return DataLoader(
            **get_dataloader_kwargs(dataset=self.test_dataset,
                                    sampling_mode=self.eval_sampling_mode,
                                    dataset_mode=DatasetMode.TESTING,
                                    dataset_config=self.dataset_config,
                                    batch_size=self.overall_batch_size_eval,
