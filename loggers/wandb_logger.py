...
class WandbLogger(Logger):
    ...
    def get_checkpoint(self,
                       artifact_name: str,
                       artifact_filepath: Optional[Path] = None) -> Path:
        ...
        if self._experiment is None:
            ...
            if wandb.run is not None:
                ...
                if self._config_args is not None:
                    self._experiment.config.update(self._config_args,
                                                   allow_val_change=True)

                # define default x-axis
                if isinstance(self._experiment,
                              (Run, RunDisabled)) and getattr(
    ...
    def log_images(self,
        ...
        Optional kwargs are lists passed to each image (ex: caption, masks,
        boxes).
        ...
        if not isinstance(images, list):
            raise TypeError(
                f'Expected a list as "images", found {type(images)}')
        n = len(images)
        for k, v in kwargs.items():
            if len(v) != n:
                raise ValueError(
                    f'Expected {n} items but only found {len(v)} for {k}')
        kwarg_list = [{k: kwargs[k][i]
                       for k in kwargs.keys()} for i in range(n)]
        metrics = {
            key: [
                wandb.Image(img, **kwarg)
                for img, kwarg in zip(images, kwarg_list)
            ]
        }
    ...
    def log_videos(self,
        ...
        for idx, video in enumerate(videos):
            ...
            wandb_videos.append(
                wandb.Video(data_or_path=video,
                            caption=caption,
                            fps=fps,
                            format=format_))
    ...
    def after_save_checkpoint(
        ...
        if self._log_model:
            self._scan_and_log_checkpoints(
                checkpoint_callback, self._save_last
                and not self._save_last_only_final)
    ...
    def _scan_and_log_checkpoints(
        ...
        checkpoints = sorted(
            ((Path(path).stat().st_mtime, path, score)
             for path, score in checkpoints.items() if Path(path).is_file()),
            key=lambda x: x[0])
        ...
        timeout = 20
        time_spent = 0
        while self._num_logged_artifact(
        ) < num_ckpt_logged_before + num_new_cktps:
            time.sleep(1)
            time_spent += 1
            if time_spent >= timeout:
                ...
                print(
                    f'self._num_logged_artifact() = {self._num_logged_artifact()}'
                )
